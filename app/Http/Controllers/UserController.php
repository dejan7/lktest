...
class UserController extends ResourceController
{
     ...
     * Attach Role
     *
     * Attach the Role to existing resource.
     *
        ...
        try {
            if ($item = $this->roleRepo->find($role_id)) {
                ...
                return ApiResponse::ItemAttached(
                    $this->repo->find($id)->attachRole($model, $data)->getModel(),
                    $this->transformer
                );
            } else {
     ...
     *
     * Attach the SocialNetwork to existing resource.
     *
     ...
     * @return \Illuminate\Http\Response
     */
    public function attachSocialNetwork(UserAttachSocialNetworkRequest $request, $id, $social_network_id)
    {
        try {
            if ($item = $this->socialNetworkRepo->find($social_network_id)) {
                ...
                return ApiResponse::ItemAttached(
                    $this->repo->find($id)->attachSocialNetwork($model, $data)->getModel(),
                    $this->transformer
                );
            } else {
     ...
     *
     * Attach the FavoriteMovie to existing resource.
     *
     ...
     * @return \Illuminate\Http\Response
     */
    public function attachFavoriteMovie(UserAttachFavoriteMovieRequest $request, $id, $movie_id)
    {
        try {
            if ($item = $this->movieRepo->find($movie_id)) {
                ...
                return ApiResponse::ItemAttached(
                    $this->repo->find($id)->attachFavoriteMovie($model, $data)->getModel(),
                    $this->transformer
                );
            } else {
     ...
     *
     * Attach the WishlistMovie to existing resource.
     *
     ...
     * @return \Illuminate\Http\Response
     */
    public function attachWishlistMovie(UserAttachWishlistMovieRequest $request, $id, $movie_id)
    {
        try {
            if ($item = $this->movieRepo->find($movie_id)) {
                $data = $request->only(array_keys($request->rules()));
                ...
                return ApiResponse::ItemAttached(
                    $this->repo->find($id)->attachWishlistMovie($model, $data)->getModel(),
                    $this->transformer
                );
            } else {
                return ApiResponse::ItemNotFound($this->movieRepo->getModel());
     ...
     * Detach Role
     *
     * Detach the Role from existing resource.
     *
     * @param  int  $id
     * @param  int  $role_id
     * @return \Illuminate\Http\Response
    ...
    public function detachRole($id, $role_id)
    {
        try {
            if ($item = $this->roleRepo->find($role_id)) {
                ...
                return ApiResponse::ItemDetached($this->repo->find($id)->detachRole($model)->getModel());
            } else {
                return ApiResponse::ItemNotFound($this->roleRepo->getModel());
            }
        } catch (QueryException $e) {
     ...
     *
     * Detach the SocialNetwork from existing resource.
     *
     * @param  int  $id
     * @param  int  $social_network_id
     * @return \Illuminate\Http\Response
     */
    public function detachSocialNetwork($id, $social_network_id)
    {
        try {
            if ($item = $this->socialNetworkRepo->find($social_network_id)) {
                $model = $item->getModel();

                return ApiResponse::ItemDetached($this->repo->find($id)->detachSocialNetwork($model)->getModel());
            } else {
                return ApiResponse::ItemNotFound($this->socialNetworkRepo->getModel());
            }
        } catch (QueryException $e) {
     ...
     *
     * Detach the FavoriteMovie from existing resource.
     *
     * @param  int  $id
     * @param  int  $movie_id
     * @return \Illuminate\Http\Response
     */
    public function detachFavoriteMovie($id, $movie_id)
    {
        try {
            if ($item = $this->movieRepo->find($movie_id)) {
                $model = $item->getModel();

                return ApiResponse::ItemDetached($this->repo->find($id)->detachFavoriteMovie($model)->getModel());
            } else {
                return ApiResponse::ItemNotFound($this->movieRepo->getModel());
            }
        } catch (QueryException $e) {
     ...
     *
     * Detach the WishlistMovie from existing resource.
     *
     * @param  int  $id
     * @param  int  $movie_id
     * @return \Illuminate\Http\Response
     */
    public function detachWishlistMovie($id, $movie_id)
    {
        try {
            if ($item = $this->movieRepo->find($movie_id)) {
                $model = $item->getModel();

                return ApiResponse::ItemDetached($this->repo->find($id)->detachWishlistMovie($model)->getModel());
            } else {
                return ApiResponse::ItemNotFound($this->movieRepo->getModel());
            }
        } catch (QueryException $e) {
